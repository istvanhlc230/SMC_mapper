...
# ============================================================
# MAPPER SETTINGS - defaults from config.json (overridable via CLI)
# ============================================================
...
DEBUG: bool                  = True
BOS_MIN_RETRACEMENT_PCT: float = 0.382


def build_arg_parser() -> argparse.ArgumentParser:
    """Build and return the CLI argument parser for SMC_mapper."""
    parser = argparse.ArgumentParser(
        description="True SMC Mapper - Institutional Structure Engine"
    )
    parser.add_argument(
        "symbol", metavar="SYMBOL",
        help="Ticker symbol to analyze (required)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Structure history size (default: 20)"
    )
    parser.add_argument("--debug", dest="debug", action="store_true")
    parser.add_argument("--no-debug", dest="debug", action="store_false")
    parser.set_defaults(debug=True)
    parser.add_argument(
        "--provider", default="twelvedata",
        help="Data provider: twelvedata, alpaca, tiingo, demo (default: twelvedata)"
    )
    return parser
...
class EngineState:
    ...
    active_high_reference: Optional[ActiveExtremeReference] = None
    active_low_reference: Optional[ActiveExtremeReference] = None
    outside_bar: bool = False
    intrabar_sequence_evidence: str = "UNAVAILABLE"
    outside_bar_reversal: Optional[bool] = None
...
def canonical_equal_extreme(a: float, b: float) -> bool:
    """Exact canonical EQH/EQL relationship; no implementation tolerance."""
    return a == b


def classify_candle_extreme_breach(c: dict, reference_high: float, reference_low: float) -> dict:
    """Return the deterministic four-level OHLC breach taxonomy."""
    body_high = max(c["open"], c["close"])
    body_low = min(c["open"], c["close"])
    physical_up = c["high"] > reference_high
    physical_down = c["low"] < reference_low
    wick_only_up = physical_up and body_high <= reference_high
    wick_only_down = physical_down and body_low >= reference_low
    body_up = body_high > reference_high
    body_down = body_low < reference_low
    close_up = c["close"] > reference_high
    close_down = c["close"] < reference_low
    return {
        "PHYSICAL_BREACH": physical_up or physical_down,
        "WICK_ONLY_BREACH": wick_only_up or wick_only_down,
        "BODY_BREACH": body_up or body_down,
        "CLOSE_BREACH": close_up or close_down,
        "UP_WICK_ONLY": wick_only_up,
        "DOWN_WICK_ONLY": wick_only_down,
        "UP_BODY": body_up,
        "DOWN_BODY": body_down,
        "UP_CLOSE": close_up,
        "DOWN_CLOSE": close_down,
        "PROVEN_INTRABAR_CROSSING": False,
    }


def classify_outside_bar(c: dict, reference_high: float, reference_low: float) -> bool:
    """Classify the geometric Outside Bar relationship only."""
    return c["high"] > reference_high and c["low"] < reference_low


def transfer_equal_extreme_references(state: EngineState, c: dict, candle_id: int) -> tuple[bool, bool]:
    """Confirm exact EQH/EQL relationships, then transfer independent references."""
    eqh = state.active_high_reference is not None and canonical_equal_extreme(
        c["high"], state.active_high_reference.price_value
    )
    eql = state.active_low_reference is not None and canonical_equal_extreme(
        c["low"], state.active_low_reference.price_value
    )
    if eqh:
        state.active_high_reference = ActiveExtremeReference(c["high"], candle_id, "ACTIVE_HIGH")
    if eql:
        state.active_low_reference = ActiveExtremeReference(c["low"], candle_id, "ACTIVE_LOW")
    return eqh, eql
...
def initialize(candles: List[dict], highs: List[StructuralLevel],
    ...
    state.active_high_reference = ActiveExtremeReference(state.reference_high, state.reference_index, "ACTIVE_HIGH")
    state.active_low_reference = ActiveExtremeReference(state.reference_low, state.reference_index, "ACTIVE_LOW")
...
def _process_outside_bar(state: EngineState, c: dict, i: int, candles: List[dict]) -> None:
    """Process one Outside Bar without fabricating an intrabar path.

    Aggregate single-timeframe OHLC establishes the geometric relationship but
    does not prove LOW_FIRST or HIGH_FIRST. Structural interpretation is therefore
    deferred unless independent sequence evidence is supplied.
    """
    ...
    state.outside_bar = classify_outside_bar(c, ref_h, ref_l)
    ...
    # Preserve physical extremes as observations. Do not infer directional
    # pullback formation from candle color or open/close placement.
    state.reference_high = max(ref_h, c["high"])
    state.reference_low = min(ref_l, c["low"])

    dbg(
        state, i,
        "OUTSIDE BAR — sequence UNAVAILABLE from aggregate OHLC; "
        "single candle event; structural interpretation deferred.",
        c["time"],
    )
...
def run_true_smc(candles: List[dict], init_end_index: Optional[int] = None):
    ...
    for i in range(start, len(candles)):
        ...
        if not bH and not bL:
            ...
            if eqH or eqL:
                # Confirm the geometric relationship first, then transfer only the
                # corresponding reference identity. Protection is independent.
                if eqH:
                    state.active_high_reference = ActiveExtremeReference(c["high"], i, "ACTIVE_HIGH")
                    state.reference_high = c["high"]
                    state.eqh_history.append({"price": c["high"], "index": i, "mother_index": state.mother_bar_index})
                if eqL:
                    state.active_low_reference = ActiveExtremeReference(c["low"], i, "ACTIVE_LOW")
                    state.reference_low = c["low"]
                    state.eql_history.append({"price": c["low"], "index": i, "mother_index": state.mother_bar_index})
                state.reference_index = i
                dbg(state, i, f"EQUAL HIGH/LOW. Reference identity transferred to ({i}); protection unchanged.", c["time"])
...
def main():
    ...
    # Apply CLI arguments to module-level globals used by engine functions
    _self.INTERVAL = args.interval
    _self.BAR_COUNT = args.bars
    _self.HISTORY_SIZE = args.history
    _self.DEBUG = args.debug

    symbol = args.symbol

    try:
        provider = auto_detect_provider(preferred=args.provider)
    except RuntimeError as e:
        print(f"ERROR: {e}")
        return

    print(f"Provider: {provider.name}  |  Symbol: {symbol}  |  Interval: {_self.INTERVAL}  |  Bars: {_self.BAR_COUNT}")
    print()

    state = None
    try:
        req = DataRequest(symbol=symbol, interval=_self.INTERVAL, bar_count=_self.BAR_COUNT)
        ...
        r = _make_error_result(symbol, str(e))

    print_mapper_summary([r])
    if not r.error:
        print_detail(r, state)
